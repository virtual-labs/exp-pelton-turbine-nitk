...
function goToNextFunction()
{
	...
	else if(simsubscreennum == 5)
	{
		...
		if(lnt == 1)
		{
			...
			generateQuestion(q2,"Head of water (h) is calculated by: ","","h = Initial hook gauge reading - Final hook gauge reading","h = Final hook gauge reading -  Initial hook gauge reading","h = Final hook gauge reading +  Initial hook gauge reading","h = Final hook gauge reading *  Initial hook gauge reading",2,screen5Proceed,390,90,355,180);
		}
		else
			document.getElementById("nextButton").style.visibility = "visible";	}
	else if(simsubscreennum == 6 && resultCount == 0)
...
function magic()
{
	...
	else if (simsubscreennum==5)
	{
		...
		document.getElementById('can5-9').innerHTML="Initial reading (water level till crest) = "+values[x][lnt][6] +" cm" ;
		myInt = setInterval(function(){ animatearrow(); }, 500);
...
function step1andhalf()
{
	...
	// {
		...
		// generateQuestion(q1,"Selected speed value is constant throughout the experiment. Say True/False? ","","False","True",0,0,2,screen1Proceed,50,340,200,160);
	// }
	// else
		document.getElementById('nextButton').style.visibility="visible";
}
...
function step2()
{
	...
	setTimeout(function(){
	document.getElementById('can2-5').innerHTML="Weight on the pan (W) ="+values[x][lnt][2] +" kg" ;
	document.getElementById('can2-6').innerHTML="Spring balance reading (S) = "+values[x][lnt][4] +" kg" ;
	document.getElementById('can2-7').innerHTML="Torque (&tau;) = ";
	idInput = document.getElementById('can2-7');
	userCalculation(idInput);
	}, 3000);
...
{
	//document.getElementById('can2-10').innerHTML="x = "+x;
	//document.getElementById('can2-11').innerHTML="y = "+y[x].text ;
	myStopFunction();
	document.getElementById("can2-8").style.transformOrigin = "20% 75%";
	document.getElementById("can2-8").style.animation = "springreading 0.9s forwards ";
	...
	setTimeout(function(){
	document.getElementById('can2-5').innerHTML="Dead weight of the pan (W) ="+values[x][lnt][2] +" kg" ;
	document.getElementById('can2-6').innerHTML="Spring balance reading (S) = "+values[x][lnt][4] +" kg" ;
	document.getElementById('can2-7').innerHTML="Torque (&tau;) = ";
		idInput = document.getElementById('can2-7');
		userCalculation(idInput);
...
function step3()
{
	...
	setTimeout(function(){
	document.getElementById('can3-4').innerHTML="Speed of motor (N) = "+y[x].text +" rpm" ;
	document.getElementById('nextButton').style.visibility="visible";
	}, 2000);
}
...
function step4()
{
	...
	setTimeout(function(){
	document.getElementById('can4-4').innerHTML="Delivery pressure = "+values[x][lnt][0] +" kg/cm<sup>2</sup> " ;
	document.getElementById('can4-5').innerHTML="Delivery head (H<sub>d</sub>) = "+values[x][lnt][1] +" m" ;
	// document.getElementById('can4-6').innerHTML="Total head(H), (H = H<sub>d</sub>+ z)= "+values[x][lnt][3] +" m" ;
	// document.getElementById('can4-6').innerHTML="Total head(H), (H = H<sub>d</sub>+ z)= "+values[x][lnt][3] +" m" ;
	// document.getElementById('nextButton').style.visibility="visible";
	document.getElementById('can4-6').innerHTML="Total head (H) = ";
...
function step5quarter()
{
	...
	document.getElementById('can5-10').innerHTML="Final reading = "+values[x][lnt][7] +" cm" ;
	document.getElementById('can5-11').innerHTML="Head of water(h) = "+values[x][lnt][8] + " cm";
...
function step6()
{
	document.getElementById("can6-3").innerHTML ="Discharge, Q ="+ values[x][lnt][9]+"m<sup>3</sup>/sec";
	document.getElementById("can6-4").innerHTML = "Total head (H) = "+values[x][lnt][3] +"m" ;
	document.getElementById("can6-4a").innerHTML = "Speed of motor (N) = "+y[x].text +"rpm" ;
	document.getElementById("can6-4b").innerHTML = "Torque (&tau;) = "+values[x][lnt][5]+"N-m";
	if(lnt == 0)
	{
		var hval = values[x][lnt][3] +"m";
		var q1 = Object.create(questions);
		generateQuestion(q1,"Total Head (H) value obtained is : ","","4.5m","2m","3.5m",hval,4,screen6Proceed,400,350,310,170);
	}
	else if(lnt == 2)
	{
		var hval = y[x].text+"rpm";
		var q1 = Object.create(questions);
		generateQuestion(q1,"Constant speed (N) value set is : ","","500rpm","600rpm","100rpm",hval,4,screen6Proceed,400,350,310,170);
	}
	else
	{
		setTimeout(function(){
	...
	$("#chartContainer").ejChart(
        {
		    ...
		    primaryXAxis:
            {
                ...
                range: { min: 0, max: 500, interval: 50 }
            },
	...
	$("#chartContainer").ejChart(
        {
            ...
            {
			    labelFormat: "{value}",
                title: { text: 'Discharge(Cumecs)' },
                range: { min: 0.00050, max: 0.00450, interval: 0.00050 }
            },
			...
			[
			    {
                points: [
				{ x: values[x][0][9], y: values[x][0][12]},
				{ x: values[x][1][9], y: values[x][1][12]},
				{ x: values[x][2][9], y: values[x][2][12]},
				{ x: values[x][3][9], y: values[x][3][12]},
				{ x: values[x][4][9], y: values[x][4][12]},
				{ x: values[x][5][9], y: values[x][5][12]},
				{ x: values[x][6][9], y: values[x][6][12]},
				{ x: values[x][7][9], y: values[x][7][12]}
	...
	$("#chartContainer").ejChart(
        {
			...
			[
			    {
                points: [
				{ x: values[x][0][11], y: values[x][0][9]},
				{ x: values[x][1][11], y: values[x][1][9]},
				{ x: values[x][2][11], y: values[x][2][9]},
				{ x: values[x][3][11], y: values[x][3][9]},
				{ x: values[x][4][11], y: values[x][4][9]},
				{ x: values[x][5][11], y: values[x][5][9]},
				{ x: values[x][6][11], y: values[x][6][9]},
				{ x: values[x][7][11], y: values[x][7][9]}
...
function refresh1()
{
	...
	document.getElementById('can2-6').innerHTML="Spring balance reading (S)= ";
	document.getElementById('can2-7').innerHTML="Torque (&tau;) = ";
	document.getElementById("can2-8").style.transformOrigin = "";
	document.getElementById("can2-8").style.animation = "";

	//document.getElementById('can2-10').innerHTML="x = ";
	//document.getElementById('can2-11').innerHTML="y = ";


	document.getElementById("can3-3").style.transformOrigin = "";
	document.getElementById("can3-3").style.animation = "";
	document.getElementById('can3-4').innerHTML="Speed of motor (N)= ";
	...
	document.getElementById('can4-5').innerHTML="Delivery head (H<sub>d</sub>) = ";
	document.getElementById('can4-6').innerHTML="Total head(H)= ";
